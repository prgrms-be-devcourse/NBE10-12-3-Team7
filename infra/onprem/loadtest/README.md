...
| | 맥 로컬 (오버레이) | 노트북 (LAN) |
|---|---|---|
| `BASE_URL` | `http://nginx:80` (컨테이너 내부) | `http://192.168.200.167` (⚠️ DHCP 로 바뀐다) |
...
> **다른 머신에서 부하를 걸 때는 [REMOTE-RUN.md](REMOTE-RUN.md) 를 볼 것.**
> 절차뿐 아니라 이 환경의 제약(양쪽 Wi-Fi 대역폭), 첫 회차를 무엇으로 잡을지와 그 이유,
> 벽이 네트워크인지 앱인지 부하 생성기인지 가르는 법, 기록의 구멍까지 적어뒀다.
...
| **1** | `s01-arrival.js` | (기본) | 비로그인 목록 — **완료**. 초당 600 까지 무릎 없음 |
| **2** | `s01-arrival.js` | `REGION_CODE=` | 동네 필터 — **완료**. 무릎 초당 200~230 |
| **3** | `s01-arrival.js` | `LOGIN=true` | 로그인 사용자 — 준비 완료, 미실행 |
| **W1** | `s02-product-create.js` | (기본) | 상품 등록 — **완료**. 초당 600 까지 무릎 없음 |
| **W2** | `s02-product-create.js` | `CEILING=true` | 쓰기 상한 — **완료**. 무릎 초당 1,000~1,200 |

회차 결과는 `results/<날짜>-<시나리오>-<메모>/summary.md` 에 있다.
...
| 항목 | 기준 |
|---|---|
| **판정** | `latency_ratio` p95 < **3배** — 그 회차 `setup()` 이 잰 무부하 대비 |
| 실패율 | < 1% |
| 자동 중단 | 배수 **10배** 초과 시(`ABORT_RATIO`). Breakpoint 는 넘는 지점을 찾는 것이 목적이라 확실히 무너진 뒤 끊는다 |
| 429 | 만나면 **즉시 중단** — 그 상태의 기록은 남기지 않는다 |
| 구간 폐기 | VM CPU 90% 초과 또는 `dropped_iterations` 발생 구간 |

**기준선을 고정값으로 두지 않는다.** 측정 경로가 바뀌면(컨테이너/LAN/유선) 같은 앱이어도
무부하가 달라진다. `setup()` 이 회차마다 직접 재고 **배수로 판정**하므로 경로가 달라도
같은 잣대가 된다.
...
scripts/
├── lib/
│   ├── config.js    기준선 측정 · 배수 판정 · 429 중단 규약
│   ├── screens.js   화면 = API 묶음   ← 이 폴더의 핵심
│   ├── stages.js    계단(도착률·VU·쓰기·상한탐색) · think time
│   └── auth.js      시드 계정 토큰 발급 (s01·s02 공용)
├── s00-ratelimit.js
├── s01-arrival.js          읽기 — REGION_CODE·LOGIN 스위치
└── s02-product-create.js   쓰기 — CEILING 스위치

scenarios/run.sh          회차 실행 + 조건 자동 수집 + 결과 보관
setup/stack-conditions.sh 맥의 스택 조건 스냅샷(원격 회차용)
setup/unload-write.sh     쓰기 회차가 만든 [load-write] 정리
results/                  회차 기록 (README.md · TEMPLATE-run.md)
```

**회차는 `scenarios/run.sh` 로 돌린다.** k6 는 요약을 고정 경로에 써서 다음 회차가 이전
회차를 덮어쓰고, 조건을 손으로 적으면 반드시 빠뜨린다. 자세한 것은 `results/README.md`.
...
**예상이 맞았다.** 읽기·쓰기 두 시나리오가 독립적으로 같은 서명을 냈다.

| 회차 | 무릎 | 무릎 시점 pending / active | 그때 VM CPU |
|---|---|---|---|
| 읽기 · 동네 필터 | 초당 200~230 | 0 → 187 / **10 고정** | 57 % |
| 쓰기 · 등록 | 초당 1,000~1,200 | 0 → 186 / **10 고정** | 68 % |

**둘 다 커넥션 풀(10)이 먼저 말랐다.** CPU 도 디스크도 락도 아니었다.
무릎 위치가 5배 차이나는데 그 방향이 **요청당 쿼리 수**(18.5 대 9.0)와 일치한다 —
커넥션 점유 시간이 상한을 결정한다. 자세한 것은 `../perf/FINDINGS.md` 의 **F-05**.
